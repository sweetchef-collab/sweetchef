...
export default function Page() {
  ...
  useEffect(() => {
    ...
    async function loadRC() {
      try {
        ...
        console.error('Recharts import failed', e);
      }
    }
    async function loadData() {
      ...
      if (!cancelled) {
        ...
        setRows(out);
        const withData = out.filter(r => (r.icham + r.terrain + r.televente + r.autre) > 0);
        const last = withData.length ? withData[withData.length - 1].mois : (out.length ? out[out.length - 1].mois : null);
        setMonth(last);
        setStatus('');
        setLoading(false);
      }
  ...
  const lineData = rows.map(r => ({ mois: r.mois, total_ht: Math.round(r.total_ht) }));
  ...
  const pieData = useMemo(() => {
    const r = rows.find(x => x.mois === month);
    if (!r) return [] as any[];
    return [
      { name: 'Icham', value: Math.round(r.icham) },
      { name: 'Terrain', value: Math.round(r.terrain) },
      { name: 'Télévente', value: Math.round(r.televente) },
      { name: 'Autre', value: Math.round(r.autre) },
    ];
  }, [rows, month]);

  const fmt = (v: any) => new Intl.NumberFormat('fr-FR').format(Math.round(v || 0));
  const gotoMonth = (m: string) => router.push(`/kpi-sans-vendeur/${encodeURIComponent(m)}`);
  const gotoPole = (name: string) => {
    const slug = name === 'Icham' ? 'ICHAM' : name === 'Terrain' ? 'TERRAIN' : name === 'Télévente' ? 'TELEVENTE' : 'AUTRE';
    router.push(`/pole/${slug}`);
  };
  ...
  return (
    <div className="container">
      ...
      <div className="panel">
        <h1 className="title">KPI — Graphiques</h1>
        <p className="subtitle">Total HT mensuel et répartition par pôle (source: vente_vendeur).</p>
        <div className="segmented" aria-label="Filtre pôle">
        ...
        <div className="grid" style={{ gridTemplateColumns: '1fr', gap: 16 }}>
              ...
              {RC ? (
                <RC.ResponsiveContainer width="100%" height="100%">
                  <RC.LineChart data={lineDataFiltered} margin={{ top: 10, right: 20, left: 80, bottom: 0 }}>
                    ...
                    <RC.Legend />
                    <RC.Line type="monotone" dataKey="value" stroke="#ff7f50" strokeWidth={2} dot={{ r: 2 }} />
                  </RC.LineChart>
              ...
              {RC ? (
                <RC.ResponsiveContainer width="100%" height="100%">
                  <RC.BarChart data={barData} margin={{ top: 10, right: 20, left: 80, bottom: 0 }}>
                    ...
                    {selectedPole == null && (
                      <>
                        <RC.Bar dataKey="Icham" stackId="a" fill="#e67e22" onClick={(d:any)=> { setMonth(d?.payload?.mois ?? month); setSelectedPole('Icham'); }} />
                        <RC.Bar dataKey="Terrain" stackId="a" fill="#2980b9" onClick={(d:any)=> { setMonth(d?.payload?.mois ?? month); setSelectedPole('Terrain'); }} />
                        <RC.Bar dataKey="Télévente" stackId="a" fill="#27ae60" onClick={(d:any)=> { setMonth(d?.payload?.mois ?? month); setSelectedPole('Télévente'); }} />
                        <RC.Bar dataKey="Autre" stackId="a" fill="#8e44ad" onClick={(d:any)=> { setMonth(d?.payload?.mois ?? month); setSelectedPole('Autre'); }} />
                      </>
                    )}
          ...
          <div className="section">
            <div className="subtitle" style={{ display:'flex', alignItems:'center', gap:8 }}>
              <span>Répartition par pôle — </span>
              <select className="input" value={month ?? ''} onChange={(e)=>setMonth(e.target.value || null)}>
                {rows.map(r => (<option key={r.mois} value={r.mois}>{r.mois}</option>))}
              </select>
            </div>
            <div style={{ width: '100%', height: 320 }}>
              {RC ? (
                <RC.ResponsiveContainer width="100%" height="100%">
                  <RC.PieChart>
                    <RC.Pie data={pieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={120} labelLine={false} label={(entry:any)=> fmt(entry.value)} onClick={(e:any)=> e?.name && setSelectedPole(e.name)}>
                      {pieData.map((_, i) => (
                        <RC.Cell key={`c-${i}`} fill={["#e67e22","#2980b9","#27ae60","#8e44ad"][i % 4]} />
                      ))}
                    </RC.Pie>
